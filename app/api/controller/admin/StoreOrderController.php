...
use crmeb\business\order\StoreOrder as StoreOrderBusiness;
...
//            return app('json')->fail('权限不足');
    ...
    public function lst(Request $request)
    {
        ...
        return app('json')->successful((new StoreOrderBusiness)->tidyOrder($data));
    }
    ...
    public function detail(Request $request, $orderId)
    {
        ...
        $orderInfo = (new StoreOrderBusiness)->tidyOrder([$order], true)[0];
    ...
    public function delivery_gain(Request $request, $orderId)
    {
        ...
        if (!$order) return app('json')->fail('订单不存在');
        if ($order['paid']) {
            $userInfo = User::getUserInfo($order['uid'], 'nickname');
            if (!empty($userInfo)) {
    ...
    public function remark(Request $request)
    {
        ...
        $order = StoreOrder::getAdminOrderDetail($order_id,'id,remark');
        if(!$order) return app('json')->fail('订单不存在');
