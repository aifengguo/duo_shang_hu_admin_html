...
class StoreOrderController
{
    ...
    public function confirm(Request $request)
    {
        ...
        $data['yue_pay_status'] = (int)sys_config('yue_pay_status') ?? (int)2;//余额支付 1 开启 2 关闭
    ...
    public function again(Request $request)
    {
        ...
        foreach ($order['cartInfo'] as $v) {
            ...
            else $res[] = StoreCart::setCart($request->uid(), $v['product_id'], $v['cart_num'], isset($v['productInfo']['attrInfo']['unique']) ? $v['productInfo']['attrInfo']['unique'] : '', 'product', 1, 0);
        }
        $cateId = [];
        foreach ($res as $v) {
