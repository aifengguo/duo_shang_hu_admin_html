...
class WechatUser extends BaseModel
{
    ...
    {
        if ($value) return date('Y-m-d H:i', $value);
        return $value;
    ...
    public static function systemPage($where = array(), $isall = false)
    {
        ...
        if (isset($where['type']) && $where['type'] !== '') $model = $model->where('user_type', $where['type']);
        $model = $model->order('uid desc');
        $count = $model->count();
        $list = $model->page((int)$where['page'], (int)$where['limit'])
            ->field('uid,openid,nickname,headimgurl,sex,country,province,city,subscribe')
            ->select()
            ->each(function ($item) {
...
//                    $item['tag_name'] = [];
//                }else{
//                    $tagId = explode(',',$item['tagid_list']);
//                    if(!$tagList || $item['tagid_list'] == ''|| !$tagId){
//                        $item['tag_name'] = [];
//                    }else{
//                        foreach ($tagId as $k=>$vo){
//                            if (isset($tagList[$vo])){
//                                $item['tag_name'][$k] = $tagList[$vo];
////                                $item['tag_name'][$k]['name'] = $tagList[$vo];
//                            }
//                        }
//                    }
//                }
...
//                    $item['group_name'] = '无';
//                }else{
//                    if(!$groupList || $item['groupid'] == 0 || !isset($groupList[$item['groupid']])){
//                        $item['group_name'] = '无';
//                    }else{
//                        $item['group_name'] = $groupList[$item['groupid']];
//                    }
//                }
     ...
     * @return array
     */
    public static function exportData($where = array(), $isall = false)
    {
        $model = new self;
        $model = $model->where('openid|routine_openid', 'NOT NULL');
        if ($where['nickname'] !== '') $model = $model->where('nickname', 'LIKE', "%$where[nickname]%");
    ...
    public static function setSpreadWhere($where = [], $alias = 'a', $model = null)
    {
        ...
        $model = self::getModelTime($where, $model, "{$alias}add_time");
    ...
    public static function agentSystemPage($where = [])
    {
        ...
        $count = $model->count();
        $list = $model->field(['avatar as headimgurl', 'brokerage_price as new_money', 'u.*', 'o.*', 'e.*', 'b.*'])
     ...
     * @return array
     */
    public static function exportAgentData($where = [])
    {
        //提现数据
        $exteactSql = UserExtract::where(['status' => 1])
            ->group('uid')
    ...
    * */
    ...
    {
        $model = self::alias('a')->join('user u', 'a.uid = u.uid')->order('u.uid desc')->where('u.is_promoter', 1);
        if ($where) {
            if (isset($where['data']) && $where['data'])
                $model = self::getModelTime($where, $model, 'a.add_time');
            if (isset($where['nickname']) && $where['nickname']) {
                $model = $model->where('u.nickname|a.uid|u.phone', 'like', "%$where[nickname]%");
            }
        }
        $uids = $model->column('u.uid');
        //分销员人数
