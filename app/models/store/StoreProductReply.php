...
class StoreProductReply extends BaseModel
{
    ...
    public static function productReplyCount($productId)
    {
        ...
        $data['reply_chance'] = $data['sum_count'] == 0 ? 100 : bcmul($data['reply_chance'], 100, 2);
        return $data;
    }
    ...
    public static function sysPage($where)
    {
        ...
        $model = $model->alias('r')
            ...
            ->join('wechat_user u', 'u.uid=r.uid', 'left');
        ...
        $model = $model->field('r.*,p.store_name,p.image,u.nickname || r.nickname as account,FROM_UNIXTIME(r.add_time,"%Y-%m-%d %H:%i:%s") as add_time');
