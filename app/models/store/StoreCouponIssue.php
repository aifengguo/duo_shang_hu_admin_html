...
class StoreCouponIssue extends BaseModel
{
    ...
    public static function issueUserCoupon($id, $uid)
    {
        ...
        if ($issueCouponInfo['remain_count'] <= 0 && !$issueCouponInfo['is_permanent']) return self::setErrorInfo('抱歉优惠券已经领取完了！');
        self::beginTrans();
    ...
    {
        $add_time = time();
        return self::create(compact('cid', 'start_time', 'end_time', 'total_count', 'remain_count', 'is_permanent', 'status', 'is_give_subscribe', 'is_full_give', 'full_reduction', 'add_time'));
    }
