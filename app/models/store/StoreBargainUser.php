...
class StoreBargainUser extends BaseModel
{
    ...
    {
        $currentBargain = self::getBargainUserCurrent(0); //TODO 获取当前用户正在砍价的商品
        $bargainProduct = StoreBargain::validBargainNumber(); //TODO 获取正在开启的砍价商品编号
        $closeBargain = [];
    ...
    {
        $currentBargain = self::getBargainUserCurrent($uid); //TODO 获取当前用户正在砍价的商品
        $bargainProduct = StoreBargain::validBargainNumber(); //TODO 获取正在开启的砍价商品编号
        $closeBargain = [];
        foreach ($currentBargain as $key => &$item) {
            if (!in_array($item, $bargainProduct)) {
