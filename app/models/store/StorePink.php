...
//                $firstWeChat = '亲，您的拼团已经完成了';
                ...
                $wechatTemplate->sendOrderPinkSuccess($item, $keyword1WeChat, $keyword2WeChat, $pid);
     ...
     * @param $pid
     */
    public static function orderPinkAfterNo($uid, $pid, $fillTilt = '', $isRemove = false)
    {
        ...
        if ($isRemove) {
            if ($openid) {//公众号发送模板消息
                $wechatTemplate->sendOrderPinkClone($uid, $pink, $store->title);
            ...
            if ($openid) {//公众号发送模板消息
                $wechatTemplate->sendOrderPinkFial($uid, $pink, $store->title);
            } else if ($routineOpenid) {//小程序发送模板消息
    ...
    public static function systemPage($where)
    {
        ...
        $model = self::getModelTime($where, $model, "p.add_time");
    ...
    public static function createPink($order)
    {
        ...
        if ($product) {
            if ($order['pink_id']) {//拼团存在
                ...
                if ($openid) { //公众号模板消息
                    $wechatTemplate = new WechatTemplate();
                    $wechatTemplate->sendOrderPinkUseSuccess($order['uid'], $order['order_id'], $product->title, $order['pink_id']);
                ...
                if ($openid && $order['is_channel'] != 1) { //公众号模板消息
                    $wechatTemplate = new WechatTemplate();
                    $wechatTemplate->sendOrderPinkOpenSuccess($order['uid'], $pink, $product->title);
            ...
            Log::error('拼团支付成功读取商品数据失败订单号：' . $order['order_id']);
        }
    }
    ...
    public static function removePink($uid, $cid, $pink_id, $nextPinkT = null)
    {
        ...
        try {
            ...
            if (StoreOrder::orderApplyRefund(StoreOrder::getPinkOrderId($pinkT['order_id_key']), $pinkT['uid'], '拼团取消开团') && self::isTpl($pinkT['uid'], $pinkT['id'])) {
                self::orderPinkAfterNo($pinkT['uid'], $pinkT['id'], '拼团取消开团', true);
            } else
    ...
    public static function sendPinkTemplateMessageError(array $pinkUidList, $pink)
    {
        foreach ($pinkUidList as $key => &$item) {
            ...
            if ($openid) {
                //公众号模板消息
                $wechatTemplate = new WechatTemplate();
                $wechatTemplate->sendOrderPinkFial($item, $pink, $store->title);
            } else if ($routineOpenid) {
                //小程序模板消息
...
//                $firstWeChat = '亲，您的拼团已经完成了';
                ...
                $wechatTemplate = new WechatTemplate();
                $wechatTemplate->sendOrderPinkSuccess($item, $keyword1WeChat, $keyword2WeChat, $pink);
            } else if ($routineOpenid) {
                //小程序模板消息
