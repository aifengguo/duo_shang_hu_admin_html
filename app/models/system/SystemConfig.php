...
use app\models\system\SystemConfigTab as ConfigTabModel;
...
class SystemConfig extends BaseModel
{
    ...
    public static function createInputRule($tab_id)
    {
        ...
        $formbuider[] = Form::number('sort', '排序', 0);
...
//        $formbuider[] = Form::input('value','默认值');
    ...
    {
        $model = new self;
        $model = $model->where('config_tab_id', $where['tab_id'])->where('status', 1);
        $list = $model->page((int)$where['page'], (int)$where['limit'])
            ...
            ->each(function ($item) {
                ...
                if ($item['type'] == 'radio' || $item['type'] == 'checkbox') {
                    $item['value'] = ConfigTabModel::getRadioOrCheckboxValueInfo($item['menu_name'], $item['value']);
                }
