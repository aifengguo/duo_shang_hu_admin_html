...
class SystemUserLevel extends BaseModel
{
     ...
     * @param string $alert
     * @param null $model
     * @return $this|SystemUserLevel|null
     */
    ...
    public static function getLevelListAndGrade($leval_id, $isArray)
    {
        ...
        $list = self::setWhere()->field('name,discount,image,icon,explain,id,grade')->where('is_del', 0)->where('is_show', 1)->order('grade asc')->select();
    ...
    {
        $model = self::setWhere($where)->order('grade asc');
