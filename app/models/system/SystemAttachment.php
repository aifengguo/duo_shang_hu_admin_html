...
class SystemAttachment extends BaseModel
{
    ...
    {
        try {
            $list = self::whereTime('time', 'yesterday')->where(['module_type' => 2])->field(['att_dir', 'att_id', 'image_type'])->select();
            foreach ($list as $key => $item) {
                $upload = UploadService::init((int)$itemp['image_type']);
                if ($itemp['image_type'] == 1) {
