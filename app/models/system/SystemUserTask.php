...
class SystemUserTask extends BaseModel
{
     ...
     * @return array
     */
    public static function taskList($level_id, $where)
    {
        $fr = [];
        if ($where['name']) $fr[] = ['name', 'like', '%' . $where['name'] . '%'];
        if ($where['is_show'] != '') $fr[] = ['is_show', '=', (int)$where['is_show']];
        $list = self::where('level_id', $level_id)->where($fr)->order('sort desc,add_time desc')->page($where['page'], $where['limit'])->select();
        $list = count($list) ? $list->toArray() : [];
