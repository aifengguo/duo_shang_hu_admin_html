...
class ExportRepositories
{
     ...
     * @return mixed
     */
    public static function export($header, $title_arr, $export = [], $filename = '', $suffix = 'xlsx', $is_save = false)
    {
     ...
     * @param $data 导出数据
     */
    public static function userFinance($data = [])
    {
        $export = [];
        if (!empty($data)) {
        ...
        $suffix = 'xlsx';
        $is_save = true;
        return self::export($header, $title, $export, $filename, $suffix, $is_save);
    }
     ...
     * @param $data 导出数据
     */
    public static function userCommission($data = [])
    {
        $export = [];
        if (!empty($data)) {
            foreach ($data as &$value) {
     ...
     * @param $data 导出数据
     */
    public static function userPoint($data = [])
    {
        $export = [];
        if (!empty($data)) {
     ...
     * @param $data 导出数据
     */
    public static function userRecharge($data = [])
    {
        $export = [];
        if (!empty($data)) {
     ...
     * @param $data 导出数据
     */
    public static function userAgent($data = [])
    {
        $export = [];
        if (!empty($data)) {
     ...
     * @param $data 导出数据
     */
    public static function wechatUser($data = [])
    {
        $export = [];
        if (!empty($data)) {
            foreach ($data as $index => $item) {
     ...
     * @param $data 导出数据
     */
    public static function orderFinance($data = [])
    {
        $export = [];
        if (!empty($data)) {
     ...
     * @param $data 导出数据
     */
    public static function storeBargain($data = [])
    {
        $export = [];
        if (!empty($data)) {
            foreach ($data as $index => $item) {
        ...
        $header = ['砍价活动名称', '砍价活动简介', '砍价金额', '成本价', '每次购买的砍价商品数量', '用户每次砍价的最大金额', '用户每次砍价的最小金额',
     ...
     * @param $data 导出数据
     */
    public static function storeCombination($data = [])
    {
        $export = [];
        if (!empty($data)) {
            foreach ($data as $item) {
        ...
        $header = ['编号', '拼团名称', '原价', '拼团价', '库存', '拼团人数', '访客人数', '展现量', '参与人数', '成团数量', '销量', '商品状态', '结束时间'];
     ...
     * @param $data 导出数据
     */
    public static function storeSeckill($data = [])
    {
        $export = [];
        if (!empty($data)) {
            foreach ($data as $item) {
    ...
    public static function storeProduct($data = [])
    {
        ...
        $header = ['商品名称', '商品简介', '商品分类', '价格', '库存', '销量', '浏览量', '收藏人数'];
...
HTML;
     ...
     * @param $data 导出数据
     */
    public static function storeMerchant($data = [])
    {
        $export = [];
        if (!empty($data)) {
            foreach ($data as $index => $item) {
